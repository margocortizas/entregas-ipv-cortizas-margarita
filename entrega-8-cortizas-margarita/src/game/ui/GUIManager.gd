...
onready var mana_progress_3 = $"%ManaProgress3"
onready var stamina_progress = $"%StaminaProgress"

onready var fading_elements: Array = [hp_progress_1, mana_progress_3, stamina_progress]
...
func _on_current_player_changed(player: Player) -> void:
	...
	_on_stamina_changed(player.stamina, player.stamina)
...
func _on_mana_changed(mana: float, mana_max: float) -> void:
	mana_progress_3.max_value = mana_max
	mana_progress_3.value = mana
	_animate_fade()
